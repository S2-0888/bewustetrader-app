...
import { 
  ...
  CheckCircle, ChartPie, Warning, Crown, Megaphone
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const unsubBroadcast = onSnapshot(doc(db, "system", "broadcast"), (docSnap) => {
      if (docSnap.exists()) setBroadcastMessage(docSnap.data().message);
    });
    ...
    return () => { unsubUser(); unsubBroadcast(); unsubTrades(); unsubAccounts(); };
  }, []);
  ...
  const money = (amount) => {
      const symbol = userProfile?.baseCurrency === 'EUR' ? '€' : userProfile?.baseCurrency === 'GBP' ? '£' : '$';
      if (showMoney) return `${symbol}${Math.round(amount || 0).toLocaleString('en-US')}`;
      return '••••';
  };
  ...
  const handleBreach = async (account) => {
      if (window.confirm(`Are you sure you want to archive account ${account.firm}?`)) {
  ...
  return (
    <div style={{ padding: isMobile ? '15px' : '40px 20px', maxWidth: 1200, margin: '0 auto', paddingBottom: 100 }}>
      ...
      {broadcastMessage && (
        <div style={{ background: '#1D1D1F', color: 'white', padding: '14px 20px', borderRadius: '18px', marginBottom: '30px', display: 'flex', alignItems: 'center', gap: '12px', boxShadow: '0 10px 20px rgba(0,0,0,0.1)', border: '1px solid rgba(255,255,255,0.1)' }}>
          <Megaphone weight="fill" color="#007AFF" size={20} />
          <span style={{ fontSize: '14px', fontWeight: 600 }}>{broadcastMessage}</span>
        </div>
      )}
        ...
        <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                {userProfile?.isFounder && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'linear-gradient(135deg, #AF52DE 0%, #5856D6 100%)', color: 'white', padding: '4px 10px', borderRadius: '30px', fontSize: '9px', fontWeight: 800, boxShadow: '0 4px 10px rgba(175, 82, 222, 0.3)', marginTop: 4 }}>
                        <Crown size={12} weight="fill" /> FOUNDER 100
                    </div>
                )}
            </div>
            <p style={{ color: '#86868B', fontSize: isMobile ? '12px' : '15px', marginTop: 4 }}>Master your process, the money follows.</p>
        </div>
            ...
            <div style={{ display:'flex', gap: 10 }}>
                ...
                <div style={{ background: 'rgba(0,0,0,0.05)', padding: 4, borderRadius: 12, display: 'flex', gap: 2 }}>
                    {['7D', '30D', 'YTD', 'ALL'].map(range => (
                        <button key={range} onClick={() => setTimeRange(range)} style={{ border: 'none', background: timeRange === range ? 'white' : 'transparent', color: 'black', padding: '8px 12px', borderRadius: 9, fontSize: 10, fontWeight: 700, cursor: 'pointer' }}>{range}</button>
                    ))}
                </div>
            </div>
        </div>
      ...
      {/* SPLIT PERFORMANCE GRID */}
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1.5fr 1fr', gap: 20, marginBottom: 20 }}>
          {/* MAIN KPI CARD */}
          <div className="bento-card" style={{ background: '#1D1D1F', color: 'white', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: 25 }}>
              <div className="label-xs" style={{ color:'#86868B', display:'flex', alignItems:'center', gap:6, marginBottom: 15 }}><TrendUp weight="fill"/> NET PERFORMANCE</div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                  <div>
                      <div style={{ fontSize: isMobile ? 42 : 56, fontWeight: 900, color: totalR >= 0 ? '#30D158' : '#FF453A', lineHeight: 1 }}>{totalR > 0 ? '+' : ''}{totalR.toFixed(1)}R</div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 10 }}>
                        <div style={{ fontSize: 11, color: '#86868B', fontWeight: 700 }}>DISCIPLINE SCORE</div>
                        <div style={{ color: '#30D158', fontWeight: 800, fontSize: 14 }}>{avgDiscipline}%</div>
                      </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: 10, color: '#86868B', fontWeight: 800 }}>WINRATE</div>
                      <div style={{ fontSize: 24, fontWeight: 800 }}>{winrate}%</div>
                  </div>
              </div>
          </div>

          {/* COMPACT FORM GUIDE */}
          <div className="bento-card" style={{ padding: 20, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                  <div className="label-xs" style={{ margin: 0, display: 'flex', alignItems: 'center', gap: 6 }}><CalendarBlank weight="fill" color="#007AFF" /> RECENT FORM</div>
                  <div style={{ fontSize: 9, fontWeight: 800, color: '#86868B' }}>LAST {calendarStrip.length} DAYS</div>
              </div>
              
              <div style={{ display: 'flex', gap: 8, justifyContent: 'space-between', alignItems: 'center' }}>
                  {(calendarStrip || []).map((day, idx) => {
                      let bgColor = '#F2F2F7'; let indicator = ''; let textColor = '#86868B';
                      if (day.hasTrades) {
                          if (day.val > 0) { bgColor = '#30D158'; indicator = 'W'; textColor = 'white'; }
                          else if (day.val < 0) { bgColor = '#FF453A'; indicator = 'L'; textColor = 'white'; }
                          else { bgColor = '#8E8E93'; indicator = 'D'; textColor = 'white'; }
                      }
                      const isToday = day.date === new Date().toISOString().split('T')[0];
                      return (
                          <div key={idx} style={{ textAlign: 'center' }}>
                              <div style={{ width: isMobile ? 32 : 34, height: isMobile ? 32 : 34, borderRadius: '50%', background: bgColor, color: textColor, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 900, border: isToday ? '2.5px solid #007AFF' : 'none', transform: isToday ? 'scale(1.1)' : 'none' }}>
                                  {indicator || '•'}
                              </div>
                              <div style={{ fontSize: 8, fontWeight: 800, color: isToday ? '#007AFF' : '#C7C7CC', marginTop: 8 }}>{day.dayNum}</div>
                          </div>
                      );
                  })}
              </div>
      ...
      {/* REMAINING STATS */}
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : '1fr 1fr', gap: 15, marginBottom: 20 }}>
          <div className="bento-card" style={{ padding: 20 }}>
              <div className="label-xs"><Scales weight="fill"/> PROFIT FACTOR</div>
              <div style={{ fontSize: 28, fontWeight: 800, marginTop: 5 }}>{winLossRatio}</div>
          </div>
          <div className="bento-card" style={{ padding: 20 }}>
              ...
              {(() => {
                ...
                if (tradesWithMfe.length === 0) return <div style={{ fontSize: 12, color: '#86868B', marginTop: 5 }}>No data.</div>;
                const score = Math.round((tradesWithMfe.reduce((acc, t) => acc + (t.pnl / t.mfe), 0) / tradesWithMfe.length) * 100);
                return <div style={{ fontSize: 28, fontWeight: 800, marginTop: 5 }}>{score}%</div>;
              })()}
          </div>
      </div>

      {/* TOP MISTAKES */}
      <div className="bento-card" style={{ marginBottom: 30 }}>
          ...
          <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 8, marginTop: 10 }}>
              {(() => {
                  ...
                  if (sortedMistakes.length === 0) return <div style={{ fontSize: 12, color: '#86868B' }}>Clean process.</div>;
                  return sortedMistakes.map(([name, count]) => (
                      <div key={name} style={{ background: 'rgba(255, 59, 48, 0.05)', border: '1px solid rgba(255, 59, 48, 0.1)', padding: '8px 12px', borderRadius: 10, flex: 1 }}>
                          <div style={{ fontSize: 9, fontWeight: 800, color: '#FF3B30' }}>{count}X</div>
      ...
      {avgDiscipline < 70 && closedTrades.length >= 3 && (
          <div className="bento-card" style={{ background: 'linear-gradient(90deg, #FF9F0A 0%, #FF3B30 100%)', color: 'white', marginBottom: 20, padding: '15px', border: 'none' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div style={{ fontSize: 12, fontWeight: 600 }}>DISCIPLINE ALERT: Review rules before next session.</div>
              </div>
          </div>
      )}
              ...
              return (
                  <div key={acc.id} className="bento-card" style={{ padding: 20 }}>
                      <div style={{ display:'flex', justifyContent:'space-between', marginBottom: 15 }}>
                          <div><div style={{ fontWeight: 800, fontSize: 16 }}>{acc.firm}</div><div style={{ fontSize: 9, color: '#007AFF', fontWeight: 800 }}>{(acc.stage || 'N/A').toUpperCase()}</div></div>
                          <div style={{ textAlign:'right', fontSize: 18, fontWeight: 800 }}>{money(currentBal)}</div>
                      </div>
                      <div style={{ display:'grid', gap:12 }}>
                          <div>
                              <div style={{ display:'flex', justifyContent:'space-between', fontSize:9, fontWeight:800, marginBottom:4 }}><span>PROFIT TARGET</span><span>{progressPct.toFixed(1)}%</span></div>
                              <div style={{ width:'100%', height:6, background:'#F2F2F7', borderRadius:3, overflow:'hidden' }}><div style={{ width:`${progressPct}%`, height:'100%', background: '#30D158' }}></div></div>
                          </div>
                          <div>
                              <div style={{ display:'flex', justifyContent:'space-between', fontSize:9, fontWeight:800, marginBottom:4 }}><span>MAX DRAWDOWN</span><span style={{color: ddPct > 80 ? '#FF3B30' : '#FF9F0A'}}>{ddPct.toFixed(1)}%</span></div>
                              <div style={{ width:'100%', height:6, background:'#F2F2F7', borderRadius:3, overflow:'hidden' }}><div style={{ width:`${ddPct}%`, height:'100%', background: ddPct > 80 ? '#FF3B30' : '#FF9F0A' }}></div></div>
                          </div>
                      </div>
